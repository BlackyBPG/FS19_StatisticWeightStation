...
StatisticWeightStation.version = "1.9.0.0"
StatisticWeightStation.date = "16.04.2021"
...
StatisticWeightStationEvent={}
StatisticWeightStationEvent_mt=Class(StatisticWeightStationEvent,Event)
InitEventClass(StatisticWeightStationEvent,"StatisticWeightStationEvent")
function StatisticWeightStationEvent:emptyNew()
	local self = Event:new(StatisticWeightStationEvent_mt)
	return self
end
function StatisticWeightStationEvent:new(barrier, StatisticWeightStationId, state, randomActive, randomStart, randomEnd, randomActiveText)
	local self = StatisticWeightStationEvent:emptyNew()
...
function StatisticWeightStationEvent:readStream(streamId, connection)
	self.barrier = NetworkUtil.readNodeObject(streamId)
	self.StatisticWeightStationId = streamReadInt32(streamId)
...
function StatisticWeightStationEvent:writeStream(streamId, connection)
	NetworkUtil.writeNodeObject(streamId,self.barrier)
	streamWriteInt32(streamId, self.StatisticWeightStationId)
...
function StatisticWeightStationEvent:run(connection)
	if g_currentMission.StatisticWeightStations ~= nil and g_currentMission.StatisticWeightStations[self.StatisticWeightStationId] ~= nil then
		g_currentMission.StatisticWeightStations[self.StatisticWeightStationId]:setOpenState(self.state, self.randomActive, self.randomStart, self.randomEnd, self.randomActiveText,true)
	end
	...
	if not connection:getIsServer() then
		g_server:broadcastEvent(StatisticWeightStationEvent:new(self.barrier, self.StatisticWeightStationId, self.state, self.randomActive, self.randomStart, self.randomEnd, self.randomActiveText), nil, connection, self.barrier)
	end
end
						...
						for i=1, #g_fillTypeManager.fillTypes do
							if wStation.measurementFillTypes[i] ~= nil and wStation.measurementFillTypes[i].mass ~= nil and wStation.measurementFillTypes[i].mass > 0 then
								...
								if not self.lineHoverActive or lineCounter < 2 or self.currentPOStart > (self.lines[lineCounter].y + self.lineHeight) then
									renderText(self.buttonsTop[3].textX, self.lines[lineCounter].textY, self.lineTextSize, string.format(g_i18n:getText("numBales"),self:formatNumbers(wStation.measurementFillTypes[i].baleCount,0)))
									renderText(self.buttonsTop[4].x, self.lines[lineCounter].textY, self.lineTextSize, string.format(g_i18n:getText("weightMass"),self:formatNumbers(wStation.measurementFillTypes[i].baleMass,3)))
								end
							end
						end
					...
					elseif self.buttonsTopState == 2 and wStation.measurementPlayer ~= nil then
						...
						for i=1, #wStation.measurementPlayer do
							if wStation.measurementPlayer[i] ~= nil and ((wStation.measurementPlayer[i].mass ~= nil and wStation.measurementPlayer[i].mass > 0) or (wStation.measurementPlayer[i].trunksMass ~= nil and wStation.measurementPlayer[i].trunksMass > 0)) then
								...
								if not self.lineHoverActive or lineCounter < 2 or self.currentPOStart > (self.lines[lineCounter].y + self.lineHeight) then
									renderText(self.buttonsTop[3].textX, self.lines[lineCounter].textY, self.lineTextSize, string.format(g_i18n:getText("weightBales"),self:formatNumbers(massBales,3)).." ("..string.format(g_i18n:getText("weightPiece"),self:formatNumbers(numBales,0))..")")
									renderText(self.buttonsTop[4].textX, self.lines[lineCounter].textY, self.lineTextSize, string.format(g_i18n:getText("weightWood"),self:formatNumbers(wStation.measurementPlayer[i].trunksMass,3)).." ("..string.format(g_i18n:getText("weightPiece"),self:formatNumbers(wStation.measurementPlayer[i].trunksCount,0))..")")
								end
							end
						end
						if hoverFillType > 0 then
